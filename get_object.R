# Gets a science metadata object (or any other kind of object, for
# that matter) from KNB and writes it to standard output.
#
# Usage: Rscript get_object.R PROD|SANDBOX pid
#
...
if (length(args) != 2 || !(args[1] %in% c("PROD", "SANDBOX"))) {
  stop("Usage: Rscript get_object.R PROD|SANDBOX pid")
}
environment <- args[1]
pid <- args[2]
...
cat(rawToChar(getObject(mn, pid)))
